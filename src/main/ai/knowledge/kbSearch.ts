...
// R2: RRF 混合检索融合（向量 + FTS5 + 标题匹配三路并行）→ 见 kbSearchFts.ts
...
import {
  ...
  DEFAULT_VEC_SCORE_THRESHOLD,
} from './kbSearchFts';
import type { SearchCandidate } from './kbSearchFts';

// Re-export 保持向后兼容（外部模块从 kbSearch 导入）
...
export type { SearchCandidate } from './kbSearchFts';
