...
| 编辑主区 v2 重做  | [specs/editor-v2-architecture.md](./specs/editor-v2-architecture.md)         | 编辑主区深度重做（M1-M4 已完成）：muya 式块树/控制器分层/块内 contentEditable、Markdown 无损双向转换、真实 Chromium E2E 验证、已知限制 |
...
> **v2 重做已完成（2026-08-06）**：编辑主区按 marktext/muya 架构完成深度重做：
>
> - **内核**（`src/render/editor/kernel/`）：不可变块树（容器/叶子分型、链表、嵌套）、
>   Markdown 无损双向转换、行内渲染器（语法标记保留）。
> - **控制器**（`src/render/editor/controllers/`）：input/enter/backspace/convert/click/
>   list/format 七类交互，行为对齐 marktext（按需重渲染、IME 守卫、六条退出规则）。
> - **渲染**（`src/render/components/Editor/v2/`）：仅内容块 contentEditable，递归容器块。
> - **集成**：撤销/重做、大纲导航、滚动高亮、代码块语言/复制、链接打开、空块占位。
> - **验证**：Vitest 304 例 + Playwright 真实 Chromium E2E 6 例全部通过。
>
> 完整设计见 [specs/editor-v2-architecture.md](./specs/editor-v2-architecture.md)。
> v1 渲染路径保留为回退（`window.__EDITOR_V2__ === false`）。
...
- `src/render/`：React 前端（页面、Editor 组件、**editor/ 内核与控制器**、stores、services、styles）
  - `src/render/editor/`：编辑主区 v2 内核（kernel/ + controllers/）
  - `src/render/components/Editor/v2/`：v2 渲染层
  - `src/render/components/Common/CreateDialog.tsx`：新建文件/文件夹弹窗
- `src/shared/`：共享 types/constants
- `docs/`：设计与模块文档（入口见 `.claude/CLAUDE.md`）
- `tests/`：Vitest 测试
- `e2e/`：Playwright 真实 Chromium E2E（编辑输入/IME/富文本渲染）
