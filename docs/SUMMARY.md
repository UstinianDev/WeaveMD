...
- 六条退出规则 + 退格链；代码块一键删除/受保护空行（重载后经解析期补偿恢复，SPEC-EDIT-CBTP）；浮动工具栏（块类型下拉 + 格式按钮）
- 跨块鼠标拖选 + 块树级删除；**v1 回退路径已退役**（v2 唯一路径）

## 4. 验证与测试

- Vitest：238 例（内核/控制器/组件，含往返不变式、退出规则矩阵、输入链路、跨块删除、代码块尾随空行补偿）
- Playwright 真实 Chromium E2E：25 例（输入/IME/富文本渲染/语法外观/退出与退格链/
