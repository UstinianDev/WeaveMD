...
> 参考：Notus 项目知识库能力，R1~R12 全部交付（2026-08-25）

| 编号 | 需求 | 优先级 | 说明 |
|------|------|--------|------|
| KB-06 | Embedding 多提供商 | P1 | OpenAI/Qwen/Doubao/Zhipu/自定义，自适应批量（20→10→5→2→1），provider/dimension 持久化 |
| KB-07 | RRF 混合检索 | P1 | 向量+FTS5+标题三路并行 → `1/(k+rank)` 融合，k=60 |
| KB-08 | 加权策略 | P1 | 当前文件+0.04~0.12、时效+0.05、标题+0.1、置顶1.5x |
| KB-09 | 段聚合 | P1 | heading 提升+单文件 cap(3)+上下文扩展(±1chunk) |
| KB-10 | 查询理解 | P1 | 5类意图+指代消解+查询扩展+模糊检测 |
| KB-11 | 条件重排 | P1 | top2差距<0.03触发LLM reranker，5分钟缓存 |
| KB-12 | 知识澄清 | P1 | 歧义检测→澄清卡片→重新检索 |
| KB-13 | 证据分级 | P1 | grounded/weak/conflicting/no_evidence 四级 |
| KB-14 | 研究循环 | P1 | 证据不足→自动3+2子查询+内存缓存 |
