...
- **空内容退出（2026-08-06 修订）**：代码块内容为空时按 Backspace 或 Enter → **保留代码块**，
  光标移到下一个内容块（即自动补的空段落）；无后续块则回退到前一块末尾。内容非空时
  Enter 为代码内换行、Backspace 为正常删除字符。
- **代码块后空段落**：可通过 Backspace 删除（空段落被移除、光标回到代码块）；段落内容
  不会被合并进代码块（防止把正文并入代码）。
