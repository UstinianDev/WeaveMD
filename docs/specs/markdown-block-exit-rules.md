...
- **退出条件**：textarea 内容为空且按 Backspace → 撤销代码块：
  - 文档中还有其他块：删除代码块本身，光标移动到前一个块末尾（无前块则后一个块开头）。
  - 代码块是唯一块：转换为空正文块（保留输入位置）。
- **空内容回车（2026-08-06 起）**：代码块内容为空时按 Enter → 撤销代码块，光标移到
  下一内容块（即自动补的空段落）；内容非空时 Enter 仍为换行。

### 3.6 引用（blockquote）

- **渲染**：左侧绿色竖线（`border-l`），不可选中。
- **退出条件**：光标在内容开头按 Backspace → 转换为正文；空引用块按 Enter 同样转换。
